...
WS=/data/fastlio_baseline/workspaces/b1
OUT="$WS/build_acceptance"
BIN="$WS/install/fast_lio/lib/fast_lio/fastlio_mapping"
mkdir -p "$OUT"
test "$(cat "$WS/build_exit_code.txt")" = "0"
test -x "$BIN"
test -z "$(git -C /data/fastlio_baseline/source/b1_instrumented status --porcelain=v2)"
set +u
...
sha256sum "$BIN" >"$OUT/fastlio_mapping.sha256"
{
  ...
  echo "b1_commit=$(git -C /data/fastlio_baseline/source/b1_instrumented rev-parse HEAD)"
  echo "source_clean=true"
} >"$OUT/summary.txt"
echo "B1 build acceptance passed"
